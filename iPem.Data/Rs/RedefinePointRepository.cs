...
namespace iPem.Data {
    public partial class RedefinePointRepository {
        ...
        public RedefinePoint GetEntity(string devId,string potId) {
            SqlParameter[] parms = { new SqlParameter("@DeviceId", SqlDbType.VarChar, 100),
                                     new SqlParameter("@PointId", SqlDbType.VarChar, 100) };

            parms[0].Value = SqlTypeConverter.DBNullStringChecker(devId);
            parms[1].Value = SqlTypeConverter.DBNullStringChecker(potId);
        ...
        public List<RedefinePoint> GetEntities(DateTime time) {
            SqlParameter[] parms = { new SqlParameter("@UpdateTime", SqlDbType.DateTime) };
            parms[0].Value = SqlTypeConverter.DBNullDateTimeChecker(time);

            var entities = new List<RedefinePoint>();
            using (var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Rs.Sql_RedefinePoint_Repository_GetEntities2, parms)) {
                while (rdr.Read()) {
                    var entity = new RedefinePoint();
                    entity.DeviceId = SqlTypeConverter.DBNullStringHandler(rdr["DeviceId"]);
                    entity.PointId = SqlTypeConverter.DBNullStringHandler(rdr["PointId"]);
