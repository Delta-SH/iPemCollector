...
namespace iPem.Data {
    public partial class HisBatTimeRepository {
        ...
        public void DeleteEntities(DateTime start, DateTime end) {
            ...
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                var trans = conn.BeginTransaction(IsolationLevel.ReadCommitted);
                try {
                    SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_HisBatTime_Repository_DeleteEntities, parms);
                    trans.Commit();
                } catch {
                    trans.Rollback();
                    throw;
