...
namespace iPem.Data {
    public partial class H_IDeviceRepository {
        ...
        public void SaveEntities(List<H_IDevice> entities) {
            ...
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach(var entity in entities) {
                        ...
                        SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_H_IDevice_Repository_SaveEntities, parms);
                    }
                    trans.Commit();
                } catch {
                    trans.Rollback();
        ...
        public void DeleteEntities() {
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                var trans = conn.BeginTransaction(IsolationLevel.ReadCommitted);
                try {
                    SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_H_IDevice_Repository_DeleteEntities, null);
                    trans.Commit();
                } catch {
                    trans.Rollback();
                    throw;
